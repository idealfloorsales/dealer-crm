document.addEventListener('DOMContentLoaded', () => {
    console.log("CRM Loaded: main.js v1.2 (Direct Access)");
    ...
    let state = { allDealers: [], statusList: [], competitorsRef: [], fullProductCatalog: [], currentUserRole: 'guest', currentStep: 1, addPhotosData: [], editPhotosData: [], newAvatarBase64: null, isSaving: false };
    ...
    function initMapLogic(key, mapId, latId, lngId, searchId, btnSearchId, btnLocId) {
        ...
        if(maps[key]) { maps[key].remove(); maps[key] = null; markers[key] = null; } // KILL OLD MAP
        ...
        const doSearch = async () => {
            ...
            else { try { const r = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(q)}&countrycodes=kz&limit=1`); const d = await r.json(); if(d[0]) setPoint(parseFloat(d[0].lat), parseFloat(d[0].lon)); else window.showToast("Не найдено", "error"); } catch(e) {} }
        };
    ...
    function renderDealers() {
        ...
        grid.innerHTML = list.map(d => {
            const st = state.statusList.find(s=>s.value===(d.status||'standard')) || {label:d.status, color:'#777'}; const avatar = d.photo_url ? `<img src="${d.photo_url}">` : `<i class="bi bi-shop"></i>`; const editBtn = state.currentUserRole !== 'guest' ? `<button class="btn-circle" onclick="window.openEditModal('${d.id}')"><i class="bi bi-pencil"></i></button>` : '';
    ...
    function renderDashboard() { const db = getEl('dashboard-stats'); if(!db) return; const total = state.allDealers.length; db.innerHTML = `<div class="col-12"><div class="stat-card-modern p-3 text-center"><h3>${total}</h3><small>Всего дилеров</small></div></div>`; }
    ...
    if(btnAdd) btnAdd.onclick = () => {
        state.currentStep = 1; getEl('add-dealer-form').reset();
        ...
        if(getEl('step-1')) { document.querySelectorAll('.wizard-step').forEach(s=>s.classList.remove('active')); getEl('step-1').classList.add('active'); }
        const m = new bootstrap.Modal(getEl('add-modal'));
        getEl('add-modal').addEventListener('shown.bs.modal', () => refreshAddMap());
        m.show();
    };
    
    const next = getEl('btn-next-step'); if(next) next.onclick = () => { if(state.currentStep===1 && (!getVal('dealer_id') || !getVal('name'))) return window.showToast('Заполните поля!','error'); if(state.currentStep===1) refreshAddMap(); if(state.currentStep<4) { getEl(`step-${state.currentStep}`).classList.remove('active'); state.currentStep++; getEl(`step-${state.currentStep}`).classList.add('active'); } };
    const prev = getEl('btn-prev-step'); if(prev) prev.onclick = () => { if(state.currentStep>1) { getEl(`step-${state.currentStep}`).classList.remove('active'); state.currentStep--; getEl(`step-${state.currentStep}`).classList.add('active'); } };

    // Save
    const formAdd = getEl('add-dealer-form');
    if(formAdd) formAdd.onsubmit = async (e) => { e.preventDefault(); const btn = getEl('btn-finish-step'); btn.disabled=true; const data={ dealer_id:getVal('dealer_id'), name:getVal('name'), city:getVal('city'), address:getVal('address'), status:getVal('status'), responsible:getVal('responsible'), latitude:getVal('add_latitude'), longitude:getVal('add_longitude'), contacts:collectList('add-contact-list', '.contact-entry', [{key:'name',sel:'.contact-name'},{key:'contactInfo',sel:'.contact-info'}]), pos_materials:collectList('add-pos-list', '.pos-entry', [{key:'name',sel:'.pos-name'},{key:'quantity',sel:'.pos-quantity'}]), visits:collectList('add-visits-list', '.visit-entry', [{key:'date',sel:'.visit-date'},{key:'comment',sel:'.visit-comment'}]) }; try { await fetch(API.dealers, {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(data)}); window.showToast("Добавлено!"); setTimeout(()=>location.reload(), 1000); } catch(err){ window.showToast("Ошибка", "error"); btn.disabled=false; } };

    // Edit (Global)
    window.openEditModal = async (id) => {
        try { const r = await fetch(`${API.dealers}/${id}`); const d = await r.json(); getEl('edit_db_id').value=d.id; getEl('edit_dealer_id').value=d.dealer_id; getEl('edit_name').value=d.name; getEl('edit_city').value=d.city; getEl('edit_address').value=d.address; getEl('edit_latitude').value=d.latitude||''; getEl('edit_longitude').value=d.longitude||''; if(getEl('edit_status')) getEl('edit_status').value=d.status||'standard';
        renderList('edit-contact-list', d.contacts, generators.contact); renderList('edit-address-list', d.additional_addresses, generators.address); renderList('edit-pos-list', d.pos_materials, generators.pos); renderList('edit-visits-list', d.visits, generators.visit); renderList('edit-competitor-list', d.competitors, generators.competitor);
        const cl = getEl('edit-product-checklist'); if(cl) cl.innerHTML = state.fullProductCatalog.map(p => `<div class="form-check"><input type="checkbox" class="form-check-input" value="${p.id}" ${(d.products||[]).find(x=>x.id===p.id)?'checked':''}><label>${p.sku} ${p.name}</label></div>`).join('');
        const m = new bootstrap.Modal(getEl('edit-modal')); getEl('edit-modal').addEventListener('shown.bs.modal', () => refreshEditMap()); m.show(); } catch(e) { window.showToast("Ошибка", "error"); }
    };
    const formEdit = getEl('edit-dealer-form');
    if(formEdit) formEdit.onsubmit = async (e) => { e.preventDefault(); const btn=formEdit.querySelector('button'); btn.disabled=true; const id=getVal('edit_db_id'); const data={ dealer_id:getVal('edit_dealer_id'), name:getVal('edit_name'), city:getVal('edit_city'), address:getVal('edit_address'), status:getVal('edit_status'), responsible:getVal('edit_responsible'), latitude:getVal('edit_latitude'), longitude:getVal('edit_longitude'), contacts:collectList('edit-contact-list', '.contact-entry', [{key:'name',sel:'.contact-name'},{key:'contactInfo',sel:'.contact-info'}]), visits:collectList('edit-visits-list', '.visit-entry', [{key:'date',sel:'.visit-date'},{key:'comment',sel:'.visit-comment'}]) }; try { await fetch(`${API.dealers}/${id}`, {method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify(data)}); const pIds = getSelectedProducts('edit-product-checklist'); await fetch(`${API.dealers}/${id}/products`, {method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify({productIds:pIds})}); window.showToast("Сохранено!"); setTimeout(()=>location.reload(), 1000); } catch(e){ window.showToast("Ошибка", "error"); btn.disabled=false; } };

    // Logout (Safe)
    const btnLogout = getEl('logout-btn'); // ИМЯ ПЕРЕМЕННОЙ ИЗМЕНЕНО, ЧТОБЫ НЕ БЫЛО КОНФЛИКТОВ
