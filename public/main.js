document.addEventListener('DOMContentLoaded', () => {
    console.log("CRM Loaded: main.js v2.3 (Dashboard & Status Fix)");
    ...
    function collectList(listId, selector, keys) {
        ...
        el.querySelectorAll(selector).forEach(row => { const obj = {}; let hasVal = false; keys.forEach(k => { const inp = row.querySelector(k.sel); if(inp){ obj[k.key] = inp.value; if(obj[k.key]) hasVal=true; } }); if(hasVal) res.push(obj); }); return res;
    }
    ...
    function initMapLogic(key, mapId, latId, lngId, searchId, btnSearchId, btnLocId) {
        ...
        maps[key] = map;
        const setPoint = (lat, lng) => { if(markers[key]) map.removeLayer(markers[key]); markers[key] = L.marker([lat, lng], {draggable:true}).addTo(map); getEl(latId).value = lat.toFixed(6); getEl(lngId).value = lng.toFixed(6); markers[key].on('dragend', e => { const p = e.target.getLatLng(); getEl(latId).value = p.lat.toFixed(6); getEl(lngId).value = p.lng.toFixed(6); }); map.setView([lat, lng], 16); };
        map.on('click', e => setPoint(e.latlng.lat, e.latlng.lng));
        const doSearch = async () => { const q = getEl(searchId).value.trim(); if(!q) return; const coords = q.match(/^(-?\d+(\.\d+)?)[,\s]+(-?\d+(\.\d+)?)$/); if(coords) { setPoint(parseFloat(coords[1]), parseFloat(coords[3])); window.showToast("Координаты!"); } else { try { const btn = getEl(btnSearchId); const old = btn.innerHTML; btn.innerHTML = '...'; const r = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(q)}&countrycodes=kz&limit=1`); const d = await r.json(); if(d[0]) setPoint(parseFloat(d[0].lat), parseFloat(d[0].lon)); else window.showToast("Не найдено", "error"); btn.innerHTML = old; } catch(e) {} } };
    ...
    function populateModalMenus(prefix, savedPriceType = null, savedResponsible = null) {
        ...
        if (typeSelect && typeSelect.tagName === 'SELECT') { const existing = new Set(state.allDealers.map(d => d.price_type).filter(Boolean)); if(savedPriceType) existing.add(savedPriceType); if(!existing.has('ОПТ')) existing.add('ОПТ'); if(!existing.has('Розница')) existing.add('Розница'); let html = '<option value="">-- Выберите --</option>'; existing.forEach(t => html += `<option value="${t}" ${t===savedPriceType?'selected':''}>${t}</option>`); typeSelect.innerHTML = html; }
        if (respSelect && respSelect.tagName === 'SELECT') { const existing = new Set(state.allDealers.map(d => d.responsible).filter(Boolean)); if(savedResponsible) existing.add(savedResponsible); const sys = ['regional_astana', 'regional_regions', 'office']; let html = '<option value="">-- Выберите --</option>'; html += `<option value="regional_astana" ${savedResponsible==='regional_astana'?'selected':''}>Региональный Астана</option>`; html += `<option value="regional_regions" ${savedResponsible==='regional_regions'?'selected':''}>Региональный Регионы</option>`; html += `<option value="office" ${savedResponsible==='office'?'selected':''}>Офис</option>`; existing.forEach(r => { if (!sys.includes(r)) html += `<option value="${r}" ${r===savedResponsible?'selected':''}>${r}</option>`; }); respSelect.innerHTML = html; }
    }
    ...
    async function initApp() {
        ...
        // ВАЖНО: Добавлена загрузка fetchTasks()
        await Promise.all([
            fetchStatuses(), 
            fetchProducts(), 
            fetchCompetitors(), 
    ...
    // NEW: Загрузка задач для дашборда
    ...
    function populateFilters() { const c = getEl('filter-city'), p = getEl('filter-price-type'); if(!c || !p) return; const cities = [...new Set(state.allDealers.map(d=>d.city).filter(Boolean))].sort(); const prices = [...new Set(state.allDealers.map(d=>d.price_type).filter(Boolean))].sort(); c.innerHTML = '<option value="">Город</option>'; cities.forEach(x => c.add(new Option(x,x))); p.innerHTML = '<option value="">Тип цен</option>'; prices.forEach(x => p.add(new Option(x,x))); }
    ...
    function renderDealers() {
        ...
        const list = state.allDealers.filter(d => { let vis = true; if(!fStat) { const sObj = state.statusList.find(s=>s.value===(d.status||'standard')); if(sObj && sObj.isVisible===false) vis=false; } else vis = (d.status === fStat); return vis && (!fCity || d.city===fCity) && (!fType || d.price_type===fType) && (!fResp || d.responsible===fResp) && (!fSearch || (d.name+d.dealer_id).toLowerCase().includes(fSearch)); });
        if(!list.length) { grid.innerHTML = '<div class="text-center text-muted py-5">Пусто</div>'; return; }
        const salesMap = {}; state.currentMonthSales.forEach(s => { if(s.dealerId) { if(!salesMap[s.dealerId]) salesMap[s.dealerId]=0; salesMap[s.dealerId] += (s.fact||0); } });
        grid.innerHTML = list.map(d => {
            ...
            let colorClass = 'bg-danger'; if (salesVolume >= 250) colorClass = 'bg-success'; else if (salesVolume >= 100) colorClass = 'bg-warning text-dark';
            ...
            let icons = '';
            if (d.contacts && d.contacts.length > 0) { const phone = d.contacts.find(c => c.contactInfo)?.contactInfo || ''; const cleanPhone = phone.replace(/[^0-9]/g, ''); if (cleanPhone.length >= 10) { icons += `<a href="https://wa.me/${cleanPhone}" target="_blank" class="btn-circle btn-circle-wa" onclick="event.stopPropagation()"><i class="bi bi-whatsapp"></i></a>`; icons += `<a href="tel:+${cleanPhone}" class="btn-circle btn-circle-call" onclick="event.stopPropagation()"><i class="bi bi-telephone-fill"></i></a>`; } }
            if (d.latitude && d.longitude) { icons += `<a href="https://yandex.kz/maps/?pt=${d.longitude},${d.latitude}&z=17&l=map" target="_blank" class="btn-circle" onclick="event.stopPropagation()"><i class="bi bi-geo-alt-fill"></i></a>`; }
            if (d.instagram) { let url = d.instagram.trim(); if (!url.startsWith('http')) { if (url.startsWith('@')) url = 'https://instagram.com/' + url.substring(1); else url = 'https://instagram.com/' + url; } icons += `<a href="${url}" target="_blank" class="btn-circle btn-circle-insta" onclick="event.stopPropagation()"><i class="bi bi-instagram"></i></a>`; }
            return `<div class="dealer-item" onclick="window.open('dealer.html?id=${d.id}','_blank')"><div class="dealer-avatar-box">${avatar}</div><div class="dealer-content"><div class="d-flex align-items-center gap-2 mb-1"><span class="dealer-name">${safeText(d.name)}</span><span style="background:${st.color};color:#fff;padding:2px 8px;border-radius:10px;font-size:0.7em">${st.label}</span></div><div class="dealer-meta"><span>#${safeText(d.dealer_id)}</span><span><i class="bi bi-geo-alt text-muted"></i> ${safeText(d.city)}</span>${d.price_type ? `<span><i class="bi bi-tag-fill text-muted" style="font-size:0.8em"></i> ${safeText(d.price_type)}</span>` : ''}${salesBadge}</div></div><div class="dealer-actions">${icons} ${editBtn}</div></div>`;
        }).join('');
    }

    // --- DASHBOARD (ИСПРАВЛЕННЫЙ) ---
    function renderDashboard() {
        ...
        // Tasks Logic (using state.allTasksData)
        ...
        state.allTasksData.forEach(d => {
            ...
            if(d.status === 'problem') { if(!tasksProblem.some(t => t.dealerId === d.id)) tasksProblem.push({dealerName:d.name, dealerId:d.id, type:'status', comment:'Проблемный статус'}); }
        ...
        const renderTList = (arr, type) => {
            ...
            return arr.slice(0, 50).map(t => {
                ...
                return `<div class="d-flex justify-content-between align-items-center border-bottom py-2"><div class="text-truncate" style="max-width:60%"><a href="dealer.html?id=${t.dealerId}" target="_blank" class="text-decoration-none fw-bold text-dark">${safeText(t.dealerName)}</a></div><div>${badge}</div></div>`;
            }).join('');
        };
        ...
        db.innerHTML = `
            ...
            <div class="col-md-4 mt-2"><div class="stat-card-modern d-block p-3 h-100"><h6 class="text-danger fw-bold"><i class="bi bi-exclamation-circle-fill"></i> Проблемы (${tasksProblem.length})</h6><div style="max-height:200px; overflow-y:auto;">${renderTList(tasksProblem, 'problem')}</div></div></div>
            <div class="col-md-4 mt-2"><div class="stat-card-modern d-block p-3 h-100"><h6 class="text-primary fw-bold"><i class="bi bi-calendar-check-fill"></i> План (${tasksUpcoming.length})</h6><div style="max-height:200px; overflow-y:auto;">${renderTList(tasksUpcoming, 'upcoming')}</div></div></div>
            <div class="col-md-4 mt-2"><div class="stat-card-modern d-block p-3 h-100"><h6 class="text-warning fw-bold"><i class="bi bi-hourglass-split"></i> Давно не были (${tasksCooling.length})</h6><div style="max-height:200px; overflow-y:auto;">${renderTList(tasksCooling, 'cooling')}</div></div></div>
        `;
    }

    // 7. HANDLERS
    window.openEditModal = async (id) => {
        try { const r = await fetch(`${API.dealers}/${id}`); const d = await r.json(); populateModalMenus('edit', d.price_type, d.responsible); getEl('edit_db_id').value = d.id; getEl('edit_dealer_id').value = d.dealer_id; getEl('edit_name').value = d.name; getEl('edit_city').value = d.city; getEl('edit_address').value = d.address; getEl('edit_latitude').value = d.latitude||''; getEl('edit_longitude').value = d.longitude||''; if(getEl('edit_status')) getEl('edit_status').value = d.status || 'standard'; getEl('edit_organization').value = d.organization || ''; getEl('edit_delivery').value = d.delivery || ''; getEl('edit_website').value = d.website || ''; getEl('edit_instagram').value = d.instagram || ''; getEl('edit_bonuses').value = d.bonuses || ''; const pType = getEl('edit_price_type'); if(pType) pType.value = d.price_type || ''; const pResp = getEl('edit_responsible'); if(pResp) pResp.value = d.responsible || ''; renderList('edit-contact-list', d.contacts, generators.contact); renderList('edit-address-list', d.additional_addresses, generators.address); renderList('edit-pos-list', d.pos_materials, generators.pos); renderList('edit-visits-list', d.visits, generators.visit); renderList('edit-competitor-list', d.competitors, generators.competitor); state.editPhotosData = d.photos || []; renderPhotoPreviews(getEl('edit-photo-preview-container'), state.editPhotosData); const cl = getEl('edit-product-checklist'); if(cl) cl.innerHTML = state.fullProductCatalog.map(p => `<div class="form-check"><input type="checkbox" class="form-check-input" value="${p.id}" ${(d.products||[]).find(x=>x.id===p.id)?'checked':''}><label>${p.sku} ${p.name}</label></div>`).join(''); const modalEl = getEl('edit-modal'); const m = new bootstrap.Modal(modalEl, {backdrop:'static'}); modalEl.addEventListener('shown.bs.modal', () => refreshEditMap()); const tabMap = modalEl.querySelector('button[data-bs-target="#tab-map"]'); if(tabMap) tabMap.addEventListener('shown.bs.tab', () => refreshEditMap()); m.show(); } catch(e) { window.showToast("Ошибка загрузки", "error"); console.error(e); }
    };

    const btnAdd = getEl('open-add-modal-btn');
    if(btnAdd) btnAdd.onclick = () => { state.currentStep = 1; getEl('add-dealer-form').reset(); populateModalMenus('add'); ['add-contact-list','add-address-list','add-pos-list','add-visits-list','add-competitor-list'].forEach(id => getEl(id).innerHTML=''); getEl('add_latitude').value=''; getEl('add_longitude').value=''; state.addPhotosData=[]; renderPhotoPreviews(getEl('add-photo-preview-container'), []); renderProductChecklist(getEl('add-product-checklist')); if(getEl('step-1')) { document.querySelectorAll('.wizard-step').forEach(s=>s.classList.remove('active')); document.querySelectorAll('.step-circle').forEach(s=>s.classList.remove('active')); getEl('step-1').classList.add('active'); } const m = new bootstrap.Modal(getEl('add-modal'), {backdrop:'static'}); getEl('add-modal').addEventListener('shown.bs.modal', () => refreshAddMap()); m.show(); };
    const next = getEl('btn-next-step'); if(next) next.onclick = () => { if(state.currentStep === 1) { if(!getVal('dealer_id') || !getVal('name')) return window.showToast('Заполните поля!', 'error'); refreshAddMap(); } if(state.currentStep < state.totalSteps) { getEl(`step-${state.currentStep}`).classList.remove('active'); state.currentStep++; getEl(`step-${state.currentStep}`).classList.add('active'); for(let i=1; i<=state.totalSteps; i++) { const c = getEl(`step-ind-${i}`); if(i < state.currentStep) { c.classList.add('completed'); c.innerHTML='✔'; } else { c.classList.remove('completed'); c.innerHTML=i; if(i===state.currentStep) c.classList.add('active'); else c.classList.remove('active'); } } getEl('btn-prev-step').style.display = 'inline-block'; if(state.currentStep === state.totalSteps) { next.style.display = 'none'; getEl('btn-finish-step').style.display = 'inline-block'; } } };
    const prev = getEl('btn-prev-step'); if(prev) prev.onclick = () => { if(state.currentStep > 1) { getEl(`step-${state.currentStep}`).classList.remove('active'); state.currentStep--; getEl(`step-${state.currentStep}`).classList.add('active'); for(let i=1; i<=state.totalSteps; i++) { const c = getEl(`step-ind-${i}`); if(i===state.currentStep) c.classList.add('active'); else c.classList.remove('active'); } getEl('btn-next-step').style.display = 'inline-block'; getEl('btn-finish-step').style.display = 'none'; if(state.currentStep === 1) prev.style.display = 'none'; } };
    ['filter-city','filter-price-type','filter-status','filter-responsible','search-bar'].forEach(id => { const el = getEl(id); if(el) el.onchange = el.oninput = renderDealers; });
    const bindAddBtn = (btnId, listId, type) => { const b = getEl(btnId); if(b) b.onclick = () => addListItem(listId, generators[type]); };
    bindAddBtn('add-contact-btn-add-modal', 'add-contact-list', 'contact'); bindAddBtn('add-contact-btn-edit-modal', 'edit-contact-list', 'contact'); bindAddBtn('add-address-btn-add-modal', 'add-address-list', 'address'); bindAddBtn('add-address-btn-edit-modal', 'edit-address-list', 'address'); bindAddBtn('add-pos-btn-add-modal', 'add-pos-list', 'pos'); bindAddBtn('add-pos-btn-edit-modal', 'edit-pos-list', 'pos'); bindAddBtn('add-visits-btn-add-modal', 'add-visits-list', 'visit'); bindAddBtn('add-visits-btn-edit-modal', 'edit-visits-list', 'visit'); bindAddBtn('add-competitor-btn-add-modal', 'add-competitor-list', 'competitor'); bindAddBtn('add-competitor-btn-edit-modal', 'edit-competitor-list', 'competitor');

    const formAdd = getEl('add-dealer-form');
    if(formAdd) formAdd.onsubmit = async (e) => { e.preventDefault(); const btn = getEl('btn-finish-step'); const oldHtml = btn.innerHTML; btn.disabled = true; btn.innerHTML = '<span class="spinner-border spinner-border-sm"></span>'; try { const data = { dealer_id: getVal('dealer_id'), name: getVal('name'), city: getVal('city'), address: getVal('address'), status: getVal('status'), responsible: getVal('responsible'), latitude: getVal('add_latitude'), longitude: getVal('add_longitude'), price_type: getVal('price_type'), organization: getVal('organization'), delivery: getVal('delivery'), website: getVal('website'), instagram: getVal('instagram'), bonuses: getVal('bonuses'), contacts: collectList('add-contact-list', '.contact-entry', [{key:'name',sel:'.contact-name'},{key:'contactInfo',sel:'.contact-info'}]), pos_materials: collectList('add-pos-list', '.pos-entry', [{key:'name',sel:'.pos-name'},{key:'quantity',sel:'.pos-quantity'}]), competitors: collectList('add-competitor-list', '.competitor-entry', [{key:'brand',sel:'.competitor-brand'},{key:'collection',sel:'.competitor-collection'},{key:'price_opt',sel:'.competitor-price-opt'},{key:'price_retail',sel:'.competitor-price-retail'}]) }; const r = await fetch(API.dealers, {method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify(data)}); if(r.ok) { const newD = await r.json(); const pIds = getSelectedProducts('add-product-checklist'); if(pIds.length) await fetch(`${API.dealers}/${newD.id}/products`, {method:'PUT', headers:{'Content-Type':'application/json'}, body:JSON.stringify({productIds:pIds})}); window.showToast("Добавлено!"); setTimeout(()=>location.reload(), 500); } } catch(e) { window.showToast("Ошибка", "error"); btn.disabled = false; btn.innerHTML = oldHtml; } };
    ...
    if(sForm) sForm.onsubmit = async (e) => { 
        e.preventDefault(); 
        const val = getVal('st_value') || 'st_'+Math.random().toString(36).substr(2,5); 
        const body = { 
