document.addEventListener('DOMContentLoaded', () => {
    ...
    async function initApp() {
        ...
        try { 
            ...
            if(r.ok) { 
                ...
                renderDealerList(); // РЕНДЕРИМ КАРТОЧКИ
                renderDashboard(); 
            } 
        } catch(e) { 
    ...
    function renderDashboard() {
        ...
        dashboardContainer.innerHTML = `
            <div class="col-6 col-md-3"><div class="stat-card h-100 p-3 bg-white rounded shadow-sm border border-success text-center"><div class="fs-2 fw-bold text-success">${active}</div><div class="text-muted small text-uppercase fw-bold">Активных</div></div></div>
            <div class="col-6 col-md-3"><div class="stat-card h-100 p-3 bg-white rounded shadow-sm border border-danger text-center"><div class="fs-2 fw-bold text-danger">${problem}</div><div class="text-muted small text-uppercase fw-bold">Проблем</div></div></div>
            <div class="col-6 col-md-3"><div class="stat-card h-100 p-3 bg-white rounded shadow-sm border border-primary text-center"><div class="fs-2 fw-bold text-primary">${potential}</div><div class="text-muted small text-uppercase fw-bold">Лиды</div></div></div>
            <div class="col-6 col-md-3"><div class="stat-card h-100 p-3 bg-white rounded shadow-sm border border-secondary text-center"><div class="fs-2 fw-bold text-secondary">${noPhoto}</div><div class="text-muted small text-uppercase fw-bold">Без фото</div></div></div>
        `;
    ...
    async function duplicateDealer(id) {
        try {
            ...
            addModal.show(); alert("Данные скопированы. Введите новый ID.");
        } catch (e) { alert("Ошибка дублирования"); }
    }

    if(qvForm) qvForm.addEventListener('submit', async (e) => {
        e.preventDefault(); if(isSaving)return; isSaving=true; const id=document.getElementById('qv_dealer_id').value; const comment=document.getElementById('qv_comment').value; if(!id||!comment){isSaving=false;return;} try{btn.disabled=true; const r=await fetch(`${API_DEALERS_URL}/${id}`); const d=await r.json(); const visits=[...(d.visits||[]), {date:new Date().toISOString().slice(0,10),comment,isCompleted:true}]; await fetch(`${API_DEALERS_URL}/${id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({visits})}); qvModal.hide(); alert("Визит добавлен!"); }catch(e){}finally{isSaving=false;btn.disabled=false;}
    });
