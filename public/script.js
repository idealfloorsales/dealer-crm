...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // (НОВОЕ) Безопасное получение значения (чтобы не падало, если поля нет)
    const getVal = (id) => {
    ...
    editModalEl.addEventListener('shown.bs.modal', () => {
        ...
        const lat = parseFloat(getVal('edit_latitude')); const lng = parseFloat(getVal('edit_longitude'));
        if (editModalEl.markerRef.current && editMap) editMap.removeLayer(editModalEl.markerRef.current);
        if (editMap) {
    ...
    function renderPhotoPreviews(container, photosArray) { if(!container) return; container.innerHTML = photosArray.map((p, index) => `<div class="photo-preview-item"><img src="${p.photo_url}"><button type="button" class="btn-remove-photo" data-index="${index}">×</button></div>`).join(''); }
    ...
    function collectData(container, selector, fields) {
        ...
        container.querySelectorAll(selector).forEach(entry => {
            ...
            fields.forEach(f => { 
                const input = entry.querySelector(f.class);
                if(input) {
    ...
    if(openAddModalBtn) openAddModalBtn.onclick = () => {
        ...
        if(document.getElementById('add_latitude')) document.getElementById('add_latitude').value = ''; 
        if(document.getElementById('add_longitude')) document.getElementById('add_longitude').value = '';
