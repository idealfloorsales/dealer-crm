...
document.addEventListener('DOMContentLoaded', () => {
    ...
    addForm.addEventListener('submit', async (e) => {
        ...
        try {
            ...
            if (selectedProductIds.length > 0) await saveDealerProductLinks(newDealer.id, selectedProductIds);
            addModal.hide();
            await initApp(); 
        } catch (error) {
    ...
    async function openEditModal(id) {
        editProductChecklist.innerHTML = "<p>Загрузка...</p>";
        editModal.show();
        try {
            const dealerRes = await fetch(`${API_DEALERS_URL}/${id}`);
            const data = await dealerRes.json();
            const selectedProductIds = (data.products || []).map(p => p.id); 
    ...
    dealerListBody.addEventListener('click', (e) => {
        ...
        if (target.closest('.btn-view')) {
            window.open(`dealer.html?id=${target.closest('.btn-view').dataset.id}`, '_blank');
        }
        if (target.closest('.btn-edit')) {
            openEditModal(target.closest('.btn-edit').dataset.id); 
        }
