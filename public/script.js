document.addEventListener('DOMContentLoaded', () => {
    ...
    let isSaving = false; // Глобальная защита от двойного клика
    ...
    // ФУНКЦИЯ ВЫПОЛНЕНИЯ ЗАДАЧИ (С АНИМАЦИЕЙ)
    ...
    function renderTaskList(container, tasks, type) {
        if(!container) return;
        if(!tasks.length) { const msg = type === 'cooling' ? 'Никого нет' : 'Задач нет'; container.innerHTML = `<p class="text-muted text-center p-3">${msg}</p>`; return; }
        container.innerHTML = tasks.map(t => {
            ...
            else if (type === 'problem') { badge = t.type==='overdue' ? `<span class="badge bg-danger me-2">Просрочено: ${t.date.toLocaleDateString()}</span>` : `<span class="badge bg-danger me-2">Статус</span>`; if(t.type !== 'overdue') comment = '<i>Требует внимания</i>'; } 
            else if (type === 'cooling') { badge = `<span class="badge bg-warning text-dark me-2">Не были: ${t.days} дн.</span>`; comment = '<i>Нужно связаться</i>'; }
            
            return `<div class="list-group-item d-flex justify-content-between align-items-center"><div class="me-auto"><div class="d-flex align-items-center mb-1">${badge}<a href="dealer.html?id=${t.dealerId}" target="_blank" class="fw-bold text-dark text-decoration-none">${t.dealerName}</a></div><small class="text-muted" style="white-space: pre-wrap;">${comment}</small></div>${(type === 'upcoming' || (type==='problem' && t.type==='overdue')) ? `<button class="btn btn-sm btn-success btn-complete-task ms-2" data-id="${t.dealerId}" data-index="${t.visitIndex}">✔</button>` : ''}</div>`;
        }).join('');
    }
