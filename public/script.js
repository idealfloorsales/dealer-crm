document.addEventListener('DOMContentLoaded', () => {
    ...
    // ==========================================
    // 3. ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ (ГЕНЕРАТОРЫ)
    // ==========================================
    ...
    function renderDashboard() {
        ...
        let countActive = 0, countStandard = 0, countPotential = 0, countProblem = 0; allDealers.forEach(d => { if (d.status === 'active') countActive++; else if (d.status === 'standard') countStandard++; else if (d.status === 'problem') countProblem++; else if (d.status === 'potential') countPotential++; });
    ...
    if(openAddModalBtn) openAddModalBtn.onclick = () => { if(addForm) addForm.reset(); renderProductChecklist(addProductChecklist); renderList(addContactList, [], createContactEntryHTML); renderList(addAddressList, [], createAddressEntryHTML); renderList(addPosList, [], createPosEntryHTML); renderList(addVisitsList, [], createVisitEntryHTML); renderList(addCompetitorList, [], createCompetitorEntryHTML); addPhotosData = []; renderPhotoPreviews(addPhotoPreviewContainer, []); if(addAvatarPreview) { addAvatarPreview.src = ''; addAvatarPreview.style.display='none'; } newAvatarBase64 = null; addModal.show(); };
