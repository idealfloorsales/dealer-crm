document.addEventListener('DOMContentLoaded', () => {
    ...
    let currentUserPermissions = {
        can_view_dealers: false, can_view_map: false, can_view_products: false, can_view_report: false,
        can_view_sales: false, can_view_competitors: false, can_view_knowledge: false, can_view_dashboard: false,
        can_create_dealer: false, can_edit_dealer: false, can_delete_dealer: false,
        can_manage_users: false, can_export_base: false, can_export_prices: false, is_admin: false
    };
    ...
    function renderDealerList() {
        ...
        dealerGrid.innerHTML = filtered.map((d, index) => {
            ...
            const salesBadge = `<span class="badge ${salesColorClass} rounded-pill ms-2" title="Продажи за текущий месяц">${salesFact.toFixed(0)} м²</span>`;
    ...
    // --- UPDATED: STATUS LOGIC (Manual ID) ---
    function resetStatusForm() { 
        if(!statusForm) return; statusForm.reset(); 
        document.getElementById('st_id').value = ''; 
        
        // РАЗБЛОКИРОВАТЬ поле ID для нового статуса
        const valInput = document.getElementById('st_value');
        if(valInput) { valInput.value = ''; valInput.disabled = false; }
    ...
    window.editStatus = (id) => { 
        ...
        // ЗАПОЛНИТЬ и ЗАБЛОКИРОВАТЬ поле ID
        const valInput = document.getElementById('st_value');
        if(valInput) { valInput.value = s.value; valInput.disabled = true; }
    ...
    if(statusForm) { 
        statusForm.addEventListener('submit', async (e) => { 
            ...
            // БЕРЕМ ЗНАЧЕНИЕ ИЗ ПОЛЯ (или из памяти, если disabled)
            const valInput = document.getElementById('st_value');
            let value = valInput ? valInput.value.trim() : '';

            // Валидация ID
            if(!id && !value) { alert("Введите Код (ID)"); return; }
            ...
            if(id) { 
                url += `/${id}`; method = 'PUT'; 
                // При редактировании value не отправляем, оно immutable
            } else { 
                body.value = value; // Отправляем ручной ID
            ...
            try { 
                ...
                if(res.ok) { window.showToast(id ? "Обновлено" : "Создано"); resetStatusForm(); fetchStatuses(); } 
    ...
    if(window.deleteStatus === undefined) window.deleteStatus = async (id) => { if(!confirm("Удалить этот статус?")) return; try { await fetch(`${API_STATUSES_URL}/${id}`, { method: 'DELETE' }); window.showToast("Удалено"); fetchStatuses(); } catch(e) { window.showToast("Ошибка", "error"); } };
