document.addEventListener('DOMContentLoaded', () => {
    ...
    async function initApp() {
        ...
        try { 
            ...
            if(r.ok) { allDealers=await r.json(); populateFilters(allDealers); renderDealerList(); renderDashboard(); } 
        } catch(e) { 
    ...
    function renderTasks(c, tasks, type) {
        if(!c) return;
        if(!tasks.length) { c.innerHTML=`<p class="text-muted text-center p-2 small">Пусто</p>`; return; }
    ...
    // Export
    if(exportBtn) exportBtn.onclick = async () => { /* ... old logic ... */ };
