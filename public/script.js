document.addEventListener('DOMContentLoaded', () => {
    ...
    // !!! ВНИМАНИЕ: ЗДЕСЬ МЕНЯЕМ НА КОНТЕЙНЕР КАРТОЧЕК !!!
    ...
    window.updateCollections = function(select) {
        ...
        if (ref && ref.collections) {
             const sortedCols = [...ref.collections].sort((a, b) => { const typeA = (typeof a === 'object') ? a.type : 'std'; const typeB = (typeof b === 'object') ? b.type : 'std'; if (typeA === 'std' && typeB !== 'std') return 1; if (typeA !== 'std' && typeB === 'std') return -1; return 0; });
             html += sortedCols.map(col => { const colName = (typeof col === 'string') ? col : col.name; const colType = (typeof col === 'object') ? col.type : 'std'; let label = ''; if(colType.includes('eng')) icon = '🌲 '; else if(colType.includes('french')) icon = '🌊 '; else if(colType.includes('art')) icon = '🎨 '; return `<option value="${colName}">${icon}${colName}</option>`; }).join('');
        }
    ...
    // --- RENDER LIST (ИЗМЕНЕНО НА КАРТОЧКИ) ---
            ...
            <div class="dealer-item" data-id="${d.id}" data-name="${safeText(d.name)}">
                <div class="dealer-item-header">
                    ...
                    <div class="dealer-item-info" onclick="window.open('dealer.html?id=${d.id}', '_blank')">
                        <div class="dealer-item-name">${safeText(d.name)}</div>
    ...
    async function duplicateDealer(id) {
        try {
            const res = await fetch(`${API_DEALERS_URL}/${id}`); if(!res.ok) throw new Error("Ошибка"); const d = await res.json();
            addForm.reset(); document.getElementById('dealer_id').value = ''; document.getElementById('name').value = d.name + ' (Копия)'; document.getElementById('organization').value = d.organization || ''; document.getElementById('price_type').value = d.price_type || ''; document.getElementById('city').value = d.city || ''; document.getElementById('address').value = d.address || ''; document.getElementById('responsible').value = d.responsible || ''; document.getElementById('status').value = 'potential'; 
            renderList(addContactList, d.contacts || [], createContactEntryHTML); renderList(addAddressList, d.additional_addresses || [], createAddressEntryHTML); renderList(addPosList, d.pos_materials || [], createPosEntryHTML); renderList(addCompetitorList, d.competitors || [], createCompetitorEntryHTML);
    ...
    if(dealerListContainer) dealerListContainer.addEventListener('click', (e) => {
        ...
        if (t.closest('.dealer-item-info') || t.closest('.btn-view')) window.open(`dealer.html?id=${t.closest('.dealer-item').dataset.id}`, '_blank');
        if (t.closest('.btn-edit')) openEditModal(t.closest('.dealer-item').dataset.id);
        if (t.closest('.btn-delete') && confirm("Удалить?")) fetch(`${API_DEALERS_URL}/${t.closest('.dealer-item').dataset.id}`, {method:'DELETE'}).then(initApp);
        if (t.closest('.btn-duplicate')) duplicateDealer(t.closest('.dealer-item').dataset.id);
        if (t.closest('.btn-quick-visit')) {
            const btn = t.closest('.btn-quick-visit');
            document.getElementById('qv_dealer_id').value = btn.closest('.dealer-item').dataset.id;
