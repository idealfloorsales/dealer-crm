document.addEventListener('DOMContentLoaded', () => {
    ...
    let currentUserPermissions = {
        ...
        can_export_base: false, can_export_prices: false, can_manage_users: false,
        can_manage_competitors: false, is_admin: false
    };
    ...
    async function initApp() {
        try {
            ...
            await Promise.all([
                fetchStatuses(),
                fetchProductCatalog(),
                // updatePosDatalist вызывается внутри, так как она синхронная
            ]);
            updatePosDatalist();
            ...
            // 4. Отрисовка
            populateFilters(allDealers); // <-- ВОТ ОНА, ТЕПЕРЬ ОНА ЕСТЬ
            renderDashboard();
            renderDealerList();
    ...
    // --- 7. ВСПОМОГАТЕЛЬНЫЕ ФУНКЦИИ (КОТОРЫЕ БЫЛИ ПОТЕРЯНЫ) ---
    ...
    function populateStatusSelects(selectedStatus = null) { 
        ...
        `;
        const modalHtml = statusList.length > 0 ? statusList.map(s => `<option value="${s.value}" ${selectedStatus === s.value ? 'selected' : ''}>${s.label}</option>`).join('') : defaultOpts;
