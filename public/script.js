document.addEventListener('DOMContentLoaded', () => {
    ...
    // ==========================================
    // 1. АВТОРИЗАЦИЯ И НАСТРОЙКА FETCH
    // ==========================================
    ...
    // ==========================================
    // 4. API ФУНКЦИИ (ПОДНЯТЫ НАВЕРХ ДЛЯ БЕЗОПАСНОСТИ)
    // ==========================================
    ...
    async function fetchTasks() { 
        ...
        else throw new Error("Ошибка задач"); 
    }
    ...
    async function fetchCurrentMonthSales() { 
        ...
        else throw new Error("Ошибка продаж"); 
    }
    ...
    async function fetchStatuses() { 
        ...
        if(res.ok) { 
            ...
            if(typeof renderDealerList === 'function') renderDealerList(); 
        } else {
            throw new Error("Ошибка статусов"); 
        }
    }
    ...
    async function fetchProductCatalog() { 
        ...
        if (!response.ok) throw new Error("Ошибка каталога"); 
    ...
    async function fetchReclamations() {
        try {
            const res = await fetch(API_RECLAMATIONS_URL);
    ...
    async function completeTask(btn, dealerId, visitIndex) { 
        try { 
            ...
            await fetch(`${API_DEALERS_URL}/${dealerId}`, { 
                method: 'PUT', 
                headers: { 'Content-Type': 'application/json' }, 
                body: JSON.stringify({ visits: dealer.visits }) 
            }); 
            initApp(); 
    ...
    // ==========================================
    // 5. КОНФИГУРАЦИЯ ВЫГРУЗКИ (ЭКСПОРТ)
    // ==========================================
    ...
    function createContactEntryHTML(c={}) { 
        return `<div class="contact-entry">
            ...
            <button type="button" class="btn-remove-entry" onclick="this.closest('.contact-entry').remove()"><i class="bi bi-x-lg"></i></button>
        </div>`; 
    }
    
    function createAddressEntryHTML(a={}) { 
        return `<div class="address-entry">
            ...
            <button type="button" class="btn-remove-entry" onclick="this.closest('.address-entry').remove()"><i class="bi bi-x-lg"></i></button>
        </div>`; 
    }
    ...
    function createOrgInputHTML(value='') { 
        return `<div class="input-group mb-1 org-entry">
            <input type="text" class="form-control org-input" placeholder="Юр. лицо" value="${safeAttr(value)}">
            <button type="button" class="btn btn-outline-danger" onclick="this.closest('.org-entry').remove()">X</button>
        </div>`; 
    }
        ...
        if(container) {
            container.innerHTML = photosArray.map((p, index) => `
                <div class="photo-preview-item">
                    <img src="${p.photo_url}">
                    <button type="button" class="btn-remove-photo" data-index="${index}"><i class="bi bi-x"></i></button>
                </div>
            `).join(''); 
        }
    }
    ...
    // ==========================================
    // 8. ЛОГИКА ФОРМ, СПИСКОВ И СТАТУСОВ
    // ==========================================
    ...
    window.editStatus = (id) => { 
        ...
        document.getElementById('st_value').value = s.value; 
    ...
    if(document.getElementById('btn-cancel-edit-status')) {
    ...
    if(statusForm) { 
        statusForm.addEventListener('submit', async (e) => { 
            ...
            const val = document.getElementById('st_value').value.trim(); 
            ...
            const body = { value: val, label, color, isVisible }; 
    ...
    window.openSectorManagerFromModal = (prefix) => {
        ...
        if(sectorModal) sectorModal.show();
    };
    ...
    function renderList(container, data, htmlGen) { 
        if(container) container.innerHTML = (data && data.length > 0) ? data.map(htmlGen).join('') : htmlGen(); 
    }
    
    function collectOrgs(container) { 
        const orgs = []; 
        container.querySelectorAll('.org-input').forEach(inp => { 
            if(inp.value.trim()) orgs.push(inp.value.trim()); 
        }); 
        return orgs; 
    }
    ...
    function populateFilters(dealers) { 
        ...
        const sectors = [...new Set(dealers.map(d => d.region_sector).filter(Boolean))].sort(); 
        ...
        const fs = document.getElementById('filter-sector'); 
        if(fs) { 
            const currSec = fs.value; 
            fs.innerHTML = '<option value="">Сектор</option>'; 
            sectors.forEach(s => fs.add(new Option(s, s))); 
            fs.value = currSec; 
        } 
    }
    ...
    // ==========================================
    // 11. ИНИЦИАЛИЗАЦИЯ КНОПОК ДОБАВЛЕНИЯ
    // ==========================================
    const setupListBtn = (id, list, genFunc) => { 
        const btn = document.getElementById(id); 
        if(btn) btn.onclick = () => list.insertAdjacentHTML('beforeend', genFunc()); 
    };
    ...
    // ==========================================
    // 12. ЗАГРУЗКА И ПРЕДПРОСМОТР ФОТО
    // ==========================================
    if(addAvatarInput) addAvatarInput.addEventListener('change', async (e) => { 
        const file = e.target.files[0]; 
        if (file) { 
            newAvatarBase64 = await compressImage(file, 800, 0.8); 
            addAvatarPreview.src = newAvatarBase64; 
    ...
    if(addPhotoInput) addPhotoInput.addEventListener('change', async (e) => { 
        for (let file of e.target.files) addPhotosData.push({ photo_url: await compressImage(file, 1000, 0.7) }); 
    ...
    if(editPhotoInput) editPhotoInput.addEventListener('change', async (e) => { 
        for (let file of e.target.files) editPhotosData.push({ photo_url: await compressImage(file, 1000, 0.7) }); 
            ...
            locBtn.onclick = () => { 
                if (navigator.geolocation) { 
                    ...
                    navigator.geolocation.getCurrentPosition(pos => { 
                        ...
                        alert("Нет доступа к геопозиции"); 
                        locBtn.innerHTML = '<i class="bi bi-geo-alt-fill"></i>'; 
                    }); 
                } 
            }; 
    ...
    // ==========================================
    // 14. MAIN INIT (ОСНОВНАЯ ЗАГРУЗКА)
    // ==========================================
        ...
        try {
            ...
            try { 
                ...
                if (authRes.ok) { 
                    ...
                    currentUserRole = authData.user ? authData.user.role : 'guest'; 
    ...
    if(btnDashSettings) {
        btnDashSettings.onclick = () => {
            ...
            if(list) {
                list.innerHTML = `
                    <div class="form-check form-switch mb-2"><input class="form-check-input" type="checkbox" id="set-showHealth" ${dashConfig.showHealth ? 'checked' : ''}><label class="form-check-label" for="set-showHealth">Продажи тт (Актив/Спят)</label></div>
                    <div class="form-check form-switch mb-2"><input class="form-check-input" type="checkbox" id="set-showGrowth" ${dashConfig.showGrowth ? 'checked' : ''}><label class="form-check-label" for="set-showGrowth">Прирост (Новые)</label></div>
                    <div class="form-check form-switch mb-2"><input class="form-check-input" type="checkbox" id="set-showCityPen" ${dashConfig.showCityPen ? 'checked' : ''}><label class="form-check-label" for="set-showCityPen">Потенциал городов</label></div>
                    <div class="form-check form-switch mb-2"><input class="form-check-input" type="checkbox" id="set-showMatrix" ${dashConfig.showMatrix ? 'checked' : ''}><label class="form-check-label" for="set-showMatrix">Матрица (Среднее SKU)</label></div>
                    <div class="form-check form-switch mb-2"><input class="form-check-input" type="checkbox" id="set-showVisits" ${dashConfig.showVisits ? 'checked' : ''}><label class="form-check-label" for="set-showVisits">Активность (Визиты)</label></div>
                `;
            }
    ...
    // ==========================================
    // 16. ОТРИСОВКА ДАШБОРДА И СПИСКА
    // ==========================================
    function renderDashboard() {
        ...
        allDealers.forEach(d => {
            ...
            let group = '';
            if (d.region_sector === 'DIY' || d.region_sector === 'Салоны' || d.region_sector === 'Рынки' || d.responsible === 'regional_astana') {
                group = 'Астана';
            } else if (d.region_sector === 'Север' || d.region_sector === 'Запад') {
                group = 'Север';
            } else {
                group = 'Юг';
            }
        ...
        if (kpiSalesEl) kpiSalesEl.innerHTML = `${Math.round(totalSalesFact).toLocaleString('ru-RU')} <span class="fs-6 text-muted">м²</span>`;
        
        const kpiActiveEl = document.getElementById('kpi-active');
        if (kpiActiveEl) kpiActiveEl.textContent = activeCount;
        
        const kpiMatrixEl = document.getElementById('kpi-matrix');
        if (kpiMatrixEl) kpiMatrixEl.innerHTML = `${avgSKU}/${totalPossibleCatalog} <span class="fs-6 text-muted">шт</span>`;
        
        const recEl = document.getElementById('kpi-reclamations');
        if (recEl) {
            recEl.textContent = openReclamations;
            if (openReclamations === 0) { 
                recEl.classList.remove('text-danger'); 
                recEl.classList.add('text-success'); 
            }
        }
        ...
        if (typeof ApexCharts !== 'undefined') {
            ...
            const speedoOpts = {
                    ...
                    radialBar: {
                        ...
                        dataLabels: {
                            ...
                            value: { 
                                ...
                                color: '#2d3748', 
                                offsetY: -10, 
                                formatter: function (val) { return Math.round(totalSalesFact) + " м²"; } 
                            }
                        }
                    }
                },
                fill: { 
                    type: 'gradient', 
                    gradient: { shade: 'dark', type: 'horizontal', gradientToColors: ['#f6ad55'], stops: [0, 100] } 
                },
            ...
            if (speedoContainer) { 
                ...
                chartSpeedometer = new ApexCharts(speedoContainer, speedoOpts); 
                chartSpeedometer.render(); 
            }
        }
        ...
        const coolingLimit = new Date(today.getTime() - (30 * 24 * 60 * 60 * 1000)); 
    ...
    function renderTaskList(container, tasks, type) { 
        if (!container) return; 
        if (tasks.length === 0) { 
            const msg = type === 'cooling' ? 'Все посещены недавно' : 'Отличная работа, пусто!'; 
            container.innerHTML = `<div class="text-center py-4 text-muted"><i class="bi bi-emoji-smile d-block mb-2 text-success opacity-50" style="font-size: 2rem;"></i><small>${msg}</small></div>`; 
            return; 
        } 
        ...
        container.innerHTML = tasks.map(t => { 
            ...
            if (type === 'upcoming') { 
                ...
                if (t.type === 'overdue') { 
                    badgeHtml = `<span class="badge bg-danger-subtle text-danger">Просрок</span>`; 
                    metaHtml = `<span class="text-danger small fw-bold">${safeText(t.comment)}</span>`; 
                } else { 
                    badgeHtml = `<span class="badge bg-danger">Статус</span>`; 
                    metaHtml = `<span class="small text-muted">Внимание</span>`; 
                } 
            } else if (type === 'cooling') { 
                const daysStr = t.days === 999 ? 'Никогда' : `${t.days} дн.`; 
                badgeHtml = `<span class="badge bg-warning-subtle text-dark">Простой: ${daysStr}</span>`; 
                metaHtml = `<span class="text-muted small">Пора обновить полку</span>`; 
            } 
            ...
            return `<div class="d-flex justify-content-between align-items-center p-2 mb-2 bg-white rounded border shadow-sm">
                        <div style="max-width: 80%;">
                            <a href="dealer.html?id=${t.dealerId}" target="_blank" class="fw-bold text-dark text-decoration-none d-block text-truncate">${safeText(t.dealerName)}</a>
                            <div class="d-flex align-items-center gap-2 mt-1">${badgeHtml}${metaHtml}</div>
                        </div>
                        <div>${btnHtml}</div>
                    </div>`; 
        }).join(''); 
    ...
    function renderDealerList() {
        ...
        if (filtered.length === 0) { 
            dealerGrid.innerHTML = ` <div class="empty-state py-5"><i class="bi bi-search display-4 text-muted mb-3 d-block text-center"></i><h5 class="text-muted text-center">Ничего не найдено</h5></div>`; 
            return; 
        }
        ...
        if(currentMonthSales && currentMonthSales.length > 0) {
            currentMonthSales.forEach(s => { 
                if(s.dealerId) {
                    salesMap[s.dealerId] = (parseFloat(salesMap[s.dealerId]) || 0) + (parseFloat(s.fact) || 0); 
                }
            });
        }
        ...
        dealerGrid.innerHTML = filtered.map((d, index) => {
            ...
            const avatarHtml = d.photo_url ? `<img src="${d.photo_url}" style="width:40px;height:40px;border-radius:50%;object-fit:cover;">` : `<div style="width:40px;height:40px;border-radius:50%;background:#f1f5f9;display:flex;align-items:center;justify-content:center;color:#cbd5e1;"><i class="bi bi-shop"></i></div>`;
    ...
    // ==========================================
    // 17. ЛОГИКА ЭКСПОРТА И ОТКРЫТИЯ МОДАЛОК
    // ==========================================
    if(btnExportDealers) {
        btnExportDealers.onclick = () => { 
    ...
    function renderExportModal() { 
        ...
        list.innerHTML = exportColumnsConfig.map((col, index) => `
            <div class="export-option-row" onclick="toggleExportCheckbox(${index})">
                <label class="form-check-label flex-grow-1 cursor-pointer" for="exp-col-${index}">${col.label}</label>
    ...
    window.generateAndDownloadCSV = () => { 
        ...
        allDealers.forEach(dealer => { 
            const row = selectedCols.map(col => { const val = col.getValue(dealer); return cleanCsv(val); }); 
            csvContent += row.join(";") + "\n"; 
        }); 
    ...
    if(btnExportCompetitors) {
        btnExportCompetitors.onclick = () => { 
            if(!allDealers.length) return window.showToast("Нет данных", "error"); 
    ...
    async function openEditModal(id) {
        try {
            ...
            if(d.contract) { 
                document.getElementById('edit_contract_signed').checked = d.contract.isSigned || false; 
                document.getElementById('edit_contract_date').value = d.contract.date || ''; 
            } else { 
                document.getElementById('edit_contract_signed').checked = false; 
                document.getElementById('edit_contract_date').value = ''; 
            }

            editOrgList.innerHTML = '';
            if (d.organizations && d.organizations.length > 0) { 
                d.organizations.forEach(org => editOrgList.insertAdjacentHTML('beforeend', createOrgInputHTML(org))); 
            } else { 
                editOrgList.insertAdjacentHTML('beforeend', createOrgInputHTML('')); 
            }
            ...
            if(editAvatarPreview) { 
                editAvatarPreview.src = d.avatarUrl || ''; 
                editAvatarPreview.style.display = d.avatarUrl ? 'block' : 'none'; 
            }
            ...
            if(firstTabEl) { 
                const tab = new bootstrap.Tab(firstTabEl); tab.show(); 
            } 
    ...
    if(editModalEl) { 
        ...
        if(tabMapBtn) { tabMapBtn.addEventListener('shown.bs.tab', () => { if (refreshEditMap) refreshEditMap(); }); } 
    }
    ...
    if(openAddModalBtn) openAddModalBtn.onclick = () => { 
        ...
        if(document.getElementById('add_latitude')) { 
        ...
        if(addAvatarPreview) { 
            addAvatarPreview.src = ''; addAvatarPreview.style.display='none'; 
        } 
        newAvatarBase64 = null; 
        
        document.getElementById('add-org-list').innerHTML = ''; 
        addOrgList.insertAdjacentHTML('beforeend', createOrgInputHTML());
        
        document.getElementById('add_contract_signed').checked = false; 
        document.getElementById('add_contract_date').value = '';
    ...
    function showStep(step) { 
        ...
        for (let i = 1; i <= totalSteps; i++) { 
            ...
            if(!ind) continue; 
            if (i < step) { 
                ind.classList.add('completed'); ind.innerHTML = '✔'; 
            } else { 
                ind.classList.remove('completed'); ind.innerHTML = i; 
        ...
        if (nextBtn && finishBtn) { 
            if (step === totalSteps) { 
                nextBtn.style.display = 'none'; finishBtn.style.display = 'inline-block'; 
            } else { 
    ...
    if(nextBtn) nextBtn.onclick = () => { 
        if (currentStep === 1) { 
            if (!document.getElementById('dealer_id').value || !document.getElementById('name').value) { 
                window.showToast("Заполните ID и Название", "error"); return; 
            } 
    ...
    if(addForm) {
        addForm.addEventListener('submit', async (e) => { 
            ...
            const data = { 
                dealer_id: getVal('dealer_id'), 
                name: getVal('name'), 
                organizations: collectOrgs(document.getElementById('add-org-list')), 
                price_type: getVal('price_type'), 
                city: getVal('city'), 
                address: getVal('address'), 
                delivery: getVal('delivery'), 
                website: getVal('website'), 
                instagram: getVal('instagram'), 
                latitude: getVal('add_latitude'), 
                longitude: getVal('add_longitude'), 
                bonuses: getVal('bonuses'), 
                status: getVal('status'), 
                responsible: document.getElementById('responsible').value, 
                region_sector: document.getElementById('add_region_sector').value, 
                contract: { 
                    isSigned: document.getElementById('add_contract_signed').checked, 
                    date: getVal('add_contract_date') 
                }, 
            ...
            try { 
                ...
                addModal.hide(); 
                window.showToast("Дилер добавлен!"); 
                initApp(); 
            } catch (e) { 
                window.showToast("Ошибка сохранения", "error"); 
            } finally { 
    ...
    if(editForm) {
        editForm.addEventListener('submit', async (e) => { 
            ...
            const data = { 
                ...
                hasPersonalPlan: document.getElementById('edit_has_personal_plan') ? document.getElementById('edit_has_personal_plan').checked : false,
                contract: { 
                    isSigned: document.getElementById('edit_contract_signed').checked, 
                    date: getVal('edit_contract_date') 
                }, 
