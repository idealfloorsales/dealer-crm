document.addEventListener('DOMContentLoaded', () => {
    ...
    async function initApp() {
        ...
        try { 
            ...
            if(r.ok) { 
                ...
                renderDealerList(); // РЕНДЕР КАРТОЧЕК
                renderDashboard();  // РЕНДЕР ДАШБОРДА (2х2)
            } 
        } catch(e) { 
    ...
    function renderDashboard() {
        ...
        allDealers.forEach(d => {
            if (d.status === 'archive') return; const isPotential = d.status === 'potential';
            ...
            if (!hasFuture && d.status !== 'problem' && !isPotential) { if (!lastVisitDate) cooling.push({ dealerName: d.name, dealerId: d.id, days: 999 }); else if(lastVisitDate < thirtyDaysAgo) cooling.push({ dealerName: d.name, dealerId: d.id, days: Math.floor((today - lastVisitDate)/(1000*60*60*24)) }); }
        });
