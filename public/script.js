document.addEventListener('DOMContentLoaded', () => {
    ...
    async function initApp() {
        ...
        try { 
            ...
            if(r.ok) { 
                ...
                renderDealerList(); // ВЫЗОВ ФУНКЦИИ ОТОБРАЖЕНИЯ
                renderDashboard(); 
            } 
        } catch(e) { 
    ...
    if(dealerListContainer) {
        dealerListContainer.addEventListener('click', (e) => {
            ...
            if (btnEdit) { e.stopPropagation(); openEditModal(id); }
            else if (btnDel) { e.stopPropagation(); if(confirm('Удалить?')) fetch(`${API_DEALERS_URL}/${id}`, {method:'DELETE'}).then(initApp); }
            else if (btnDup) { e.stopPropagation(); duplicateDealer(id); }
            else if (btnVisit) { e.stopPropagation(); document.getElementById('qv_dealer_id').value = id; document.getElementById('qv_comment').value = ''; qvModal.show(); }
        });
    }
    ...
    if(qvForm) qvForm.addEventListener('submit', async (e) => {
        e.preventDefault(); if(isSaving)return; isSaving=true; const id=document.getElementById('qv_dealer_id').value; const comment=document.getElementById('qv_comment').value; const btn=qvForm.querySelector('button');
        if(!id||!comment){isSaving=false;return;} try{btn.disabled=true; const r=await fetch(`${API_DEALERS_URL}/${id}`); const d=await r.json(); const visits=[...(d.visits||[]), {date:new Date().toISOString().slice(0,10),comment,isCompleted:true}]; await fetch(`${API_DEALERS_URL}/${id}`,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify({visits})}); qvModal.hide(); alert("Визит добавлен!"); }catch(e){}finally{isSaving=false;btn.disabled=false;}
    });
    
    // HELPERS
    async function duplicateDealer(id) { try{const res=await fetch(`${API_DEALERS_URL}/${id}`);const d=await res.json();addForm.reset();document.getElementById('dealer_id').value='';document.getElementById('name').value=d.name+' (Копия)';renderList(addContactList, d.contacts||[], createContactEntryHTML);renderList(addAddressList, d.additional_addresses||[], createAddressEntryHTML);renderList(addPosList, d.pos_materials||[], createPosEntryHTML);renderList(addCompetitorList, d.competitors||[], createCompetitorEntryHTML);const pIds=(d.products||[]).map(p=>p.id);renderProductChecklist(addProductChecklist,pIds);currentStep=1;showStep(1);addModal.show();alert("Скопировано. Введите ID.");}catch(e){alert("Ошибка дублирования");} }
    ...
    if(exportBtn) exportBtn.onclick = async () => { /* Export Logic same as before... */ }; // Сократил, чтобы влезло, логика была в пред. ответах
