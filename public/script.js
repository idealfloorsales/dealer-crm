document.addEventListener('DOMContentLoaded', () => {
    ...
    async function openEditModal(id) {
        try {
            ...
            const editAvatarPreview = getEl('edit-avatar-preview');
            const editCurrentAvatarUrl = getEl('edit-current-avatar-url');
    ...
    function showQuickVisit(id) { 
        getEl('qv_dealer_id').value = id; getEl('qv_comment').value = ''; qvModal.show(); 
    }
    ...
    async function initApp() {
        ...
        try { const response = await fetch(API_DEALERS_URL); if (!response.ok) throw new Error(response.statusText); allDealers = await response.json(); populateFilters(allDealers); renderDashboard(); renderDealerList(); } catch (error) { if(dealerGrid) dealerGrid.innerHTML = `<div class="col-12"><div class="alert alert-danger text-center">Ошибка загрузки: ${error.message}</div></div>`; }
    ...
    // Forms
    if(openAddModalBtn) openAddModalBtn.onclick = () => { if(addForm) addForm.reset(); populateStatusSelects(); renderProductChecklist(getProductChecklistEl('add')); renderList(getEl('add-contact-list'), [], createContactEntryHTML); renderList(getEl('add-address-list'), [], createAddressEntryHTML); renderList(getEl('add-pos-list'), [], createPosEntryHTML); renderList(getEl('add-visits-list'), [], createVisitEntryHTML); renderList(getEl('add-competitor-list'), [], createCompetitorEntryHTML); if(getEl('add_latitude')) { getEl('add_latitude').value = ''; getEl('add_longitude').value = ''; } addPhotosData = []; renderPhotoPreviews(getEl('add-photo-preview-container'), []); const av = getEl('add-avatar-preview'); if(av) { av.src = ''; av.style.display='none'; } newAvatarBase64 = null; currentStep = 1; showStep(1); addModal.show(); };
    ...
    function getProductChecklistEl(mode) { return mode === 'add' ? getEl('add-product-checklist') : getEl('edit-product-checklist'); }
