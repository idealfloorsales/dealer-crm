document.addEventListener('DOMContentLoaded', () => {
    ...
    // Глобальные функции (делаем доступными для HTML)
...
// Функция показа уведомлений (НОВАЯ)
    ...
    async function initApp() {
        ...
        try { 
            ...
            if(dealerGrid) dealerGrid.innerHTML = `<div class="col-12"><p class="text-danger text-center">Ошибка загрузки: ${error.message}</p></div>`; 
        }
   ...
   // --- RENDER LIST (С кнопкой Instagram) ---
    ...
    function renderDashboard() {
        ...
        // Фикс процентов для синей полоски
    ...
    // MAP (Safe Init)
    const DEFAULT_LAT_MAP = 51.1605; const DEFAULT_LNG_MAP = 71.4704; let addMap, editMap;
    function initMap(mapId) { const el = document.getElementById(mapId); if (!el) return null; if (typeof L === 'undefined') return null; const map = L.map(mapId).setView([DEFAULT_LAT_MAP, DEFAULT_LNG_MAP], 13); L.tileLayer('https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png', { attribution: 'OSM' }).addTo(map); return map; }
    ...
    function setupMapSearch(map, inputId, suggestionsId, latId, lngId, markerRef) { const input = document.getElementById(inputId); const suggestionsBox = document.getElementById(suggestionsId); const latInput = document.getElementById(latId); const lngInput = document.getElementById(lngId); if (!input || !suggestionsBox) return; let debounceTimer; input.addEventListener('input', async () => { clearTimeout(debounceTimer); const query = input.value.trim(); if (query.length < 3) { suggestionsBox.style.display = 'none'; return; } debounceTimer = setTimeout(async () => { try { const res = await fetch(`https://nominatim.openstreetmap.org/search?format=json&q=${encodeURIComponent(query)}&countrycodes=kz`); const data = await res.json(); suggestionsBox.innerHTML = ''; if (data.length > 0) { suggestionsBox.style.display = 'block'; data.slice(0, 5).forEach(place => { const div = document.createElement('div'); div.className = 'address-suggestion-item'; div.textContent = place.display_name; div.onclick = () => { const lat = parseFloat(place.lat); const lon = parseFloat(place.lon); if (markerRef.current) markerRef.current.setLatLng([lat, lon]); else markerRef.current = L.marker([lat, lon]).addTo(map); map.setView([lat, lon], 16); if (latInput) latInput.value = lat; if (lngInput) lngInput.value = lon; input.value = place.display_name; suggestionsBox.style.display = 'none'; if(document.getElementById('city') && !document.getElementById('city').value) { const addr = place.address || {}; document.getElementById('city').value = addr.city || addr.town || addr.village || ''; } if(document.getElementById('address') && !document.getElementById('address').value) { document.getElementById('address').value = place.display_name.split(',')[0]; } }; suggestionsBox.appendChild(div); }); } else { suggestionsBox.style.display = 'none'; } } catch (e) { console.error(e); } }, 500); }); document.addEventListener('click', (e) => { if (!e.target.closest('.map-search-container')) suggestionsBox.style.display = 'none'; }); }
