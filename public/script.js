document.addEventListener('DOMContentLoaded', () => {
    ...
    const statusModalEl = document.getElementById('status-manager-modal'); const statusModal = new bootstrap.Modal(statusModalEl); const btnManageStatuses = document.getElementById('btn-manage-statuses'); const statusForm = document.getElementById('status-form'); const statusListContainer = document.getElementById('status-manager-list');
    ...
    // MAP VARS (ИСПРАВЛЕНО: Добавлены mapInstances и markerInstances)
    ...
    // ==========================================
    // 6. MAIN LOGIC (UPDATED WITH SALES FIELDS)
    // ==========================================
    ...
    async function openEditModal(id) {
        try {
            ...
            // !!! НОВЫЕ ПОЛЯ !!!
            if(document.getElementById('edit_has_personal_plan')) document.getElementById('edit_has_personal_plan').checked = d.hasPersonalPlan || false;
            // !!! НОВОЕ ПОЛЕ !!!
            if(document.getElementById('edit_sales_category')) document.getElementById('edit_sales_category').value = d.salesCategory || '';
    ...
    async function initApp() {
        ...
        await fetchStatuses(); await fetchProductCatalog(); updatePosDatalist(); try { const compRes = await fetch(API_COMPETITORS_REF_URL); if (compRes.ok) { competitorsRef = await compRes.json(); updateBrandsDatalist(); } } catch(e){}
        try { await Promise.all([fetchDealers(), fetchTasks(), fetchCurrentMonthSales()]); populateFilters(allDealers); renderDashboard(); renderDealerList(); } catch (error) { if(dealerGrid) dealerGrid.innerHTML = `Ошибка`; }
    ...
    async function fetchCurrentMonthSales() { const month = new Date().toISOString().slice(0, 7); try { const r = await fetch(`${API_SALES_URL}?month=${month}`); if(r.ok) currentMonthSales = await r.json(); } catch(e) { console.error(e); } }
    ...
    if(editForm) editForm.addEventListener('submit', async (e) => { e.preventDefault(); if (isSaving) return; isSaving = true; const btn = document.querySelector('button[form="edit-dealer-form"]'); const oldText = btn.innerHTML; btn.disabled = true; btn.innerHTML = '<span class="spinner-border spinner-border-sm"></span>'; const id = document.getElementById('edit_db_id').value; let avatarToSend = getVal('edit-current-avatar-url'); if (newAvatarBase64) avatarToSend = newAvatarBase64; 
        const data = { 
            ...
            // !!! НОВОЕ ПОЛЕ !!!
            salesCategory: document.getElementById('edit_sales_category').value,
