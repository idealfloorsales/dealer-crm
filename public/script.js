document.addEventListener('DOMContentLoaded', () => {
    console.log("App Started v1000: Clean & Safe");

    // ==========================================
    // 1. БЕЗОПАСНЫЕ УТИЛИТЫ
    // ==========================================
    ...
    function setupMapLogic(mapId, latId, lngId, searchId, btnSearchId, btnLocId, instanceKey) {
        ...
        // ИСПРАВЛЕНИЕ: Удаляем "призрак" карты, если он есть
        if (mapEl._leaflet_id) {
             mapEl._leaflet_id = null; 
             mapInstances[instanceKey] = null; // Сброс
        }
    ...
    const filterCity = getEl('filter-city'); if(filterCity) filterCity.onchange = renderDealerList; 
    const filterPriceType = getEl('filter-price-type'); if(filterPriceType) filterPriceType.onchange = renderDealerList; 
    const filterStatus = getEl('filter-status'); if(filterStatus) filterStatus.onchange = renderDealerList; 
    const filterResponsible = getEl('filter-responsible'); if(filterResponsible) filterResponsible.onchange = renderDealerList; 
    const searchBar = getEl('search-bar'); if(searchBar) searchBar.oninput = renderDealerList;
    ...
    // ==========================================
    // 7. MAIN LOGIC (STATUSES, API, INIT)
    // ==========================================
    ...
    async function openEditModal(id) {
        try {
            ...
            const editAvatarPreview = getEl('edit-avatar-preview'); const editCurrentAvatarUrl = getEl('edit-current-avatar-url');
    ...
    const logoutBtn = getEl('logout-btn'); if (logoutBtn) { logoutBtn.onclick = (e) => { e.preventDefault(); localStorage.clear(); sessionStorage.clear(); logoutBtn.innerHTML = '<span class="spinner-border spinner-border-sm"></span>'; const xhr = new XMLHttpRequest(); xhr.open("GET", "/?chk=" + Math.random(), true, "logout", "logout"); xhr.onreadystatechange = function () { if (xhr.readyState == 4) window.location.reload(); }; xhr.send(); }; }
