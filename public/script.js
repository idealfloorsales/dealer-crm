document.addEventListener('DOMContentLoaded', () => {
    ...
    const API_RECLAMATIONS_URL = '/api/reclamations'; // <--- НОВЫЙ API ДЛЯ РЕКЛАМАЦИЙ
    ...
    let chartSpeedometer = null; // График Спидометр
    let chartRings = null;       // График Кольца
    ...
    async function initApp() {
        try {
            ...
            await Promise.all([
                ...
                fetchSectors(), 
                updatePosDatalist(),
                fetchReclamations() // Загружаем рекламации для дашборда
            ]);
    ...
    // --- НОВЫЙ ДАШБОРД (SOFT UI / APEX CHARTS) ---
    function renderDashboard() {
        ...
        // 3. Рисуем Спидометр
        const GOAL_M2 = 10000; // Цель компании в метрах
    ...
    async function fetchStatuses() { const res = await fetch(API_STATUSES_URL); if(res.ok) { statusList = await res.json(); populateStatusSelects(); renderStatusManagerList(); if(typeof renderDealerList === 'function') renderDealerList(); } else throw new Error("Ошибка статусов"); }
