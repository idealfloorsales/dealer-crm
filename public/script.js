document.addEventListener('DOMContentLoaded', () => {
    ...
    addPhotoInput.addEventListener('change', async (e) => {
        ...
        for (let file of files) {
            ...
            addPhotosData.push({ photo_url: base64 });
        }
    ...
    function collectData(container, selector, fields) {
        ...
        container.querySelectorAll(selector).forEach(entry => {
            const item = {};
            let hasData = false;
            fields.forEach(f => { const val = entry.querySelector(f.class).value; item[f.key] = val; if(val) hasData=true; });
    ...
    function renderProductChecklist(container, selectedIds=[]) {
        ...
        container.innerHTML = fullProductCatalog.map(p => {
            ...
            return `<div class="checklist-item form-check"><input type="checkbox" class="form-check-input" value="${pid}" ${set.has(pid)?'checked':''}><label class="form-check-label"><strong>${p.sku}</strong> - ${p.name}</label></div>`;
        }).join('');
    }
