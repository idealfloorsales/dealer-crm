document.addEventListener('DOMContentLoaded', () => {
    console.log("App Started v261: Full Code");

    // ==========================================
    // 1. КОНФИГУРАЦИЯ И ПЕРЕМЕННЫЕ СОСТОЯНИЯ
    // ==========================================
    ...
    // Временные хранилища для редактирования
    let addPhotosData = []; 
    let editPhotosData = []; 
    ...
    // Глобальные переменные для обновления карт
    let refreshAddMap = null;
    let refreshEditMap = null;
    
    // Переменные для мастера создания (Wizard)
    let currentStep = 1; 
    const totalSteps = 4;

    // ==========================================
    // 2. ПОЛУЧЕНИЕ ЭЛЕМЕНТОВ DOM (Безопасная функция)
    // ==========================================
    ...
    const statusListContainer = getEl('status-manager-list');
    ...
    // Получение значения input (чтобы не было ошибок, если элемента нет)
    const getVal = (id) => { 
        const el = getEl(id); 
        return el ? el.value : ''; 
    };
    ...
    // Хелпер для рендера списка фото
    ...
    // Хелпер для сбора данных из форм
    ...
    // Глобальная функция обновления брендов/коллекций
    ...
    // Инициализация логики карт (но создание произойдет при первом открытии модалки)
    ...
    // ==========================================
    // 6. ОСНОВНАЯ ЛОГИКА (API и Инициализация)
    // ==========================================
    ...
    async function initApp() {
        ...
        // 3. Open Pending Edit (if reloading from another page)
    ...
    function renderDashboard() {
        ...
        allDealers.forEach(d => { 
            ...
            if (d.status === 'problem') { 
                if (!tasksProblem.some(t => t.dealerId === d.id && t.type === 'overdue')) tasksProblem.push({ dealerName: d.name, dealerId: d.id, type: 'status', comment: 'Статус: Проблемный' }); 
            } 
        ...
        renderTaskList(getEl('tasks-list-upcoming'), tasksUpcoming, 'upcoming'); 
        renderTaskList(getEl('tasks-list-problem'), tasksProblem, 'problem'); 
        renderTaskList(getEl('tasks-list-cooling'), tasksCooling, 'cooling');
    }
    ...
    function renderTaskList(container, tasks, type) { 
        ...
        container.innerHTML = tasks.map(t => { 
            ...
            if (type === 'upcoming') { 
                ...
                badgeHtml = t.isToday ? `<span class="task-badge tb-today mt-1 d-inline-block">Сегодня</span>` : `<span class="task-badge tb-future mt-1 d-inline-block">${dateStr}</span>`; 
    ...
    function renderDealerList() {
        ...
        const filtered = allDealers.filter(d => { 
            ...
            } else { isVisible = (d.status === status); }
    ...
    function renderStatusManagerList() {
        ...
        statusListContainer.innerHTML = statusList.map(s => `<tr><td><div style="width:20px;height:20px;background:${s.color};border-radius:50%;"></div></td><td class="fw-bold">${s.label}</td><td class="text-muted small">${s.value}</td><td class="text-center">${s.isVisible!==false?'<i class="bi bi-eye-fill text-success"></i>':'<i class="bi bi-eye-slash-fill text-muted"></i>'}</td><td class="text-end"><button class="btn btn-sm btn-light border me-1" onclick="editStatus('${s.id}')"><i class="bi bi-pencil"></i></button><button class="btn btn-sm btn-light border text-danger" onclick="deleteStatus('${s.id}')"><i class="bi bi-trash"></i></button></td></tr>`).join('');
    }
    ...
    async function openEditModal(id) {
        try {
            ...
            getEl('edit_db_id').value=d.id; getEl('edit_dealer_id').value=d.dealer_id; getEl('edit_name').value=d.name; getEl('edit_organization').value=d.organization; getEl('edit_price_type').value=d.price_type; getEl('edit_city').value=d.city; getEl('edit_address').value=d.address; getEl('edit_delivery').value=d.delivery; getEl('edit_website').value=d.website; getEl('edit_instagram').value=d.instagram;
            ...
            renderList(getEl('edit-contact-list'), d.contacts, createContactEntryHTML); renderList(getEl('edit-address-list'), d.additional_addresses, createAddressEntryHTML); renderList(getEl('edit-pos-list'), d.pos_materials, createPosEntryHTML); renderList(getEl('edit-visits-list'), d.visits, createVisitEntryHTML); renderList(getEl('edit-competitor-list'), d.competitors, createCompetitorEntryHTML);
