document.addEventListener('DOMContentLoaded', () => {
    ...
    const exportColumnsConfig = [
        ...
        { id: 'name', label: 'Название', isChecked: true, getValue: d => d.name },
        { id: 'orgs', label: 'Юр. лица (Организации)', isChecked: true, getValue: d => (d.organizations || [d.organization]).filter(Boolean).join('; ') },
        ...
        { id: 'status', label: 'Статус', isChecked: true, getValue: d => {
            ...
            // Если статус кастомный, ищем в statusList, иначе берем значение
        ...
        { id: 'contract_signed', label: 'Договор (Подписан)', isChecked: false, getValue: d => (d.contract && d.contract.isSigned) ? 'Да' : 'Нет' },
        { id: 'contract_date', label: 'Дата договора', isChecked: false, getValue: d => (d.contract && d.contract.date) ? d.contract.date : '' },
        { id: 'website', label: 'Сайт', isChecked: false, getValue: d => d.website },
        { id: 'instagram', label: 'Инстаграм', isChecked: false, getValue: d => d.instagram },
        ...
        { id: 'last_visit', label: 'Последний визит', isChecked: true, getValue: d => {
            ...
            // Сортируем визиты по дате (свежие первые)
    ...
    // --- EXPORT LOGIC (CUSTOMIZABLE) ---
    if(btnExportDealers) {
        btnExportDealers.onclick = () => {
            if(!allDealers || !allDealers.length) return window.showToast("Нет данных для экспорта", "error");
            renderExportModal();
        };
    }
    ...
    window.generateAndDownloadCSV = () => {
        // 1. Определяем выбранные колонки
        const selectedCols = [];
