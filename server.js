...
// ... (Авторизация и статика без изменений, пропустим для краткости) ...
...
if (ADMIN_USER && ADMIN_PASSWORD) {
    ...
    app.get('/sales.html', authMiddleware, (req, res) => res.sendFile(__dirname + '/public/sales.html')); // НОВОЕ
    app.use(express.static('public'));
} else { app.use(express.static('public')); }

const DB_CONNECTION_STRING = process.env.DB_CONNECTION_STRING;

// ... (productsToImport и старые схемы без изменений) ...
// Вставьте сюда productsToImport и productSchema, contactSchema...
const productsToImport = [ { sku: "CD-507", name: "Дуб Беленый" } ]; // (Сократил для примера, оставьте ваш полный список)
...
const salesSchema = new mongoose.Schema({
    ...
    group: String, // "north", "south", "michael"...
    dealerId: String, // ID из базы (если есть)
    dealerName: String, // Имя (для разовых или из базы)
    plan: Number,
    fact: Number,
    isCustom: { type: Boolean, default: false } // true для разовых, которых нет в базе
});
...
app.get('/api/dealers', async (req, res) => {
    try {
        ...
        res.json(dealers.map(d => ({
            ...
            photo_url: d.avatarUrl, has_photos: (d.photos && d.photos.length > 0), products_count: (d.products ? d.products.length : 0), has_pos: (d.pos_materials && d.pos_materials.length > 0), visits: d.visits, latitude: d.latitude, longitude: d.longitude, status: d.status || 'standard', responsible: d.responsible
        }))); 
...
app.get('/api/matrix', async (req, res) => {
    try {
        ...
        const matrix = allProducts.map(p => ({
            ...
            dealers: allDealers.map(d => ({ value: map.get(d._id.toString()).has(p._id.toString()) ? 1 : 0, is_pos: false }))
        }));
...
app.post('/api/sales', async (req, res) => {
    try {
        ...
        // data = array of { dealerId, dealerName, group, plan, fact, isCustom }
        
        // Удаляем старые записи за этот месяц (перезапись)
        // Это проще, чем обновлять по одному
        const dealerIdsToUpdate = data.map(d => d.dealerId).filter(Boolean);
        const customNamesToUpdate = data.filter(d => d.isCustom).map(d => d.dealerName);
        
        // Стратегия: Upsert (Обновить или Вставить)
...
// (НОВОЕ) Список стендов (нужен для server.js чтобы не потерялся)
