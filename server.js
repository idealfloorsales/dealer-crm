...
// --- ВАЖНО: Сначала объявляем функции Хелперы (До API) ---
...
// --- 2. API (ОТКРЫТЫ ДЛЯ СКРИПТА) ---
// Мы ставим их ДО защиты паролем, чтобы script.js мог получать данные без ошибок 401
...
app.get('/api/knowledge/:id', async (req, res) => {
    try { res.json(convertToClient(await Knowledge.findById(req.params.id))); }
    catch (e) { res.status(500).json({ error: e.message }); }
});

app.post('/api/knowledge', async (req, res) => {
    try { const a = new Knowledge(req.body); await a.save(); res.json(convertToClient(a)); }
    catch (e) { res.status(500).json({ error: e.message }); }
});

app.put('/api/knowledge/:id', async (req, res) => {
    try { const a = await Knowledge.findByIdAndUpdate(req.params.id, req.body); res.json(convertToClient(a)); }
    catch (e) { res.status(500).json({ error: e.message }); }
});

app.delete('/api/knowledge/:id', async (req, res) => {
    try { await Knowledge.findByIdAndDelete(req.params.id); res.json({status:'deleted'}); }
    catch (e) { res.status(500).json({ error: e.message }); }
});


// --- 3. ЗАЩИТА ПАРОЛЕМ (ТОЛЬКО ДЛЯ САЙТА) ---
// Ставим замок ПОСЛЕ всех API, но ПЕРЕД express.static
if (ADMIN_USER && ADMIN_PASSWORD) {
    app.use(basicAuth({
        users: { [ADMIN_USER]: ADMIN_PASSWORD }, 
        challenge: true, 
        unauthorizedResponse: 'Доступ к CRM запрещен. Введите логин и пароль.'
    }));
}

// Отдаем сайт (HTML/CSS/JS) только тем, кто прошел пароль
app.use(express.static('public'));


// --- СПИСОК ТОВАРОВ (Для импорта) ---
// (Я сократил список для чтения, но на сервере он должен быть полным!)
// ... PRODUCTS ARRAY HERE ...
...
async function hardcodedImportProducts() {
    try {
        ...
        if (count < productsToImport.length) {
            // Используем bulkWrite для обновления
